...
from config import device, teacher_forcing_ratio
...
# RNN decoder with Luong attention layer
...
class GreedySearchDecoder(nn.Module):
  ...
  def forward(self, input_seq, input_length, max_length):
    ...
    decoder_input = torch.ones(1, 1, device=device,
                               dtype=torch.long) * data_loader.SOS_token
...
def train(input_variable,
  ...
  decoder_input = torch.LongTensor(
      [[data_loader.SOS_token for _ in range(batch_size)]])
...
def evaluate(encoder, decoder, searcher, voc, sentence, max_length=MAX_LENGTH):
  ...
  # words -> indexes
  indexes_batch = [data_loader.indexesFromSentence(voc, sentence)]
...
def evaluateInput(encoder, decoder, searcher, voc):
  ...
  while (1):
    try:
      ...
      # Normalize sentence
      input_sentence = helpers.normalizeString(input_sentence)
      # Evaluate sentence
