...
class QAData(object):
    ...
    def load_dataset(self, tokenizer, do_return=False):
        ...
        if self.load and os.path.exists(preprocessed_path):
            ...
            questions = [
                d["question"] if d["question"].endswith("?") else d["question"] + "?"
                for d in self.data
            ]
            ...
            question_input = tokenizer.batch_encode_plus(
                questions, pad_to_max_length=True, max_length=self.args.max_input_length
            )
        ...
        self.logger.info(
            "Loaded {} examples from {} data".format(len(self.dataset), self.data_type)
        )
    ...
    def save_predictions(self, predictions):
        assert len(predictions) == len(self), (len(predictions), len(self))
        prediction_dict = {
            dp["question"]: {'pred':prediction, 'ans':dp["answer"]}
            for dp, prediction in zip(self.data, predictions)
        }
        save_path = os.path.join(
            self.args.output_dir, "{}predictions.json".format(self.args.prefix)
        )
...
class MyQADataset(Dataset):
    def __init__(
        ...
        self.in_metadata = (
            list(zip(range(len(input_ids)), range(1, 1 + len(input_ids))))
            if in_metadata is None
            else in_metadata
        )
        self.out_metadata = (
            list(
                zip(range(len(decoder_input_ids)), range(1, 1 + len(decoder_input_ids)))
            )
        ...
        assert (
            len(self.input_ids) == len(self.attention_mask) == self.in_metadata[-1][-1]
        )
        assert (
            len(self.decoder_input_ids)
            == len(self.decoder_attention_mask)
            == self.out_metadata[-1][-1]
        )
    ...
    def __getitem__(self, idx):
        ...
        return (
            self.input_ids[in_idx],
            self.attention_mask[in_idx],
            self.decoder_input_ids[out_idx],
            self.decoder_attention_mask[out_idx],
        )
